...
sort_keys = ['K', 'N', 'L', 'P', 't']
display_keys = ['K', 'N', 'L', 'P', 't', 'fill', 'iter', 'error', 'resid']



print('\\begin{tabular}{|l|l|l|l|l|l|l|l|l|}')
print('\\hline')
print('K & N & L & MPI-процессы & OpenMP потоки & fill, сек & iter, сек & error, сек & невязка \\\\')
...
fig, axs_2d = plt.subplots(1, 2)

axs = axs_2d.flat
axs_idx = 0

group_by_keys = ['K', 'N', 'L', 'P']
...
for group_key, group in df.groupby(group_by_keys):
    ...
    ax.set(xlabel = 'OpenMP threads', ylabel = 'Acceleration, times')
...
plt.savefig(RESULTS_DIR + '/omp_mpi.png', bbox_inches='tight')
plt.close()